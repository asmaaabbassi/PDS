...
def DCA_rebalancing(assets, portfolio, amount, inv_date, inv_period):
    ...
    with open(methodsFile, 'w', newline = '') as csvfile:
            ...
            writer.writerow(["Date", "Trading method", "Allocation no", "Asset", "Weight of asset", "Asset price", "Amount", "Total spending", "Investment period", "Rebalancing"])
    ...
    for index, row in portfolio_df.iterrows():
        ...
        for x in range(0, 2*inv_period):
            if x%2 == 0:
                ...
                output = []
                output.append(tuple([curr_date.strftime('%b %d, %Y'),"DCA-rebal", str(index + 1),  "stocks", row['ST'], stocks_price, stocks_sum, stocks_cumm, inv_period]))
                output.append(tuple([curr_date.strftime('%b %d, %Y'),"DCA-rebal",  str(index + 1),  "cbonds",row['CB'], cbonds_price, cbonds_sum, cbonds_cumm, inv_period]))
                output.append(tuple([curr_date.strftime('%b %d, %Y'),"DCA-rebal", str(index + 1),  "sbonds", row['PB'], sbonds_price, sbonds_sum, sbonds_cumm, inv_period]))
                output.append(tuple([curr_date.strftime('%b %d, %Y'),"DCA-rebal", str(index + 1),  "gold", row['GO'], gold_price, gold_sum, gold_cumm, inv_period]))
                output.append(tuple([curr_date.strftime('%b %d, %Y'),"DCA-rebal", str(index + 1),  "cash", row['CA'], usd_price, usd_sum, usd_cumm, inv_period]))
                ...
                stocks_sum = stocks_cumm // stocks_price
                cbonds_sum = cbonds_cumm // cbonds_price
                sbonds_sum = sbonds_cumm // sbonds_price
                gold_sum = gold_cumm // gold_price
                usd_sum = usd_cumm // usd_price
