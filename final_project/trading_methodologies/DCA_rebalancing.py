...
def DCA_rebalancing(amount, inv_date, inv_period):     
    ...
    with open(methodsFile, 'w', newline = '') as csvfile:
            ...
            writer.writerow(["Date", "Trading method", "Allocation no", "Asset", "Weight of asset", "Asset price", "Amount to buy", "Total spending", "Investment period", "Rebalancing"])
    ...
    for index, row in portfolio_df.iterrows():
        ...
        for x in range(0, 2*inv_period):
            if x%2 == 0:
                curr_date = inv_date + relativedelta(months = x/2)
                ...
                output = []
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA", str(index + 1),  "stocks", row['ST'], stocks_price, stocks_sum, stocks_cumm, inv_period, False]))
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA",  str(index + 1),  "cbonds",row['CB'], cbonds_price, cbonds_sum, cbonds_cumm, inv_period, False]))
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA", str(index + 1),  "sbonds", row['PB'], sbonds_price, sbonds_sum, sbonds_cumm, inv_period, False]))
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA", str(index + 1),  "gold", row['GO'], gold_price, gold_sum, gold_cumm, inv_period, False]))
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA", str(index + 1),  "cash", row['CA'], usd_price, usd_sum, usd_cumm, inv_period, False]))
                ...
                curr_date = inv_date + relativedelta(months = x//2, days = 14)
                ...
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA",  str(index + 1),  "cbonds",row['CB'], cbonds_price, cbonds_sum, cbonds_cumm, inv_period]))
                ...
                total_cumm = stocks_cumm + cbonds_cumm + sbonds_cumm + gold_cumm + usd_cumm
                
                stocks_cumm = stocks_rate * total_cumm
                cbonds_cumm = cbonds_rate * total_cumm
                sbonds_cumm = sbonds_rate * total_cumm
                gold_cumm = sbonds_rate * total_cumm
                usd_cumm = usd_rate * total_cumm
                
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA-rebal", str(index + 1),  "stocks", row['ST'], stocks_price, 0, stocks_cumm, inv_period]))
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA-rebal",  str(index + 1),  "cbonds",row['CB'], cbonds_price, 0, cbonds_cumm, inv_period]))
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA-rebal", str(index + 1),  "sbonds", row['PB'], sbonds_price, 0, sbonds_cumm, inv_period]))
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA-rebal", str(index + 1),  "gold", row['GO'], gold_price, 0, gold_cumm, inv_period]))
                output.append(tuple([inv_date.strftime('%b %d, %Y'),"DCA-rebal", str(index + 1),  "cash", row['CA'], usd_price, 0, usd_cumm, inv_period]))
...
print(DCA_rebalancing(100000, datetime(2020, 7, 1), 3))
